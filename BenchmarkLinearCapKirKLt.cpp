...
{
class BenchmarkLinearCapKirKLt : public fitness_t
{
	public:

	BenchmarkLinearCapKirKLt()
	{
	...
	std::string name() const { return "BenchmarkLinearCa,p+Kir+K+L"; }
};
}
...
int main(int argc, char **argv)
{
  ...
  //-----------------------------------------
  hillvallea::fitness_pt fitness_function = std::make_shared<hillvallea::BenchmarkLinearCapKirKLt>();
  ...
  std::string file_appendix = "BenchmarkLinearCapKirKLt_seed_"+std::to_string(random_seed); // can be used when multiple runs are outputted in the same directory
  ...
  // Running HillVallEA
  std::cout << "Running HillVallEA on the Multimodal BenchmarkLinearCa,p+Kir+K+L" << std::endl;
